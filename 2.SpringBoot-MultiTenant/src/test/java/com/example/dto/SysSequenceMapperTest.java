...
public class SysSequenceMapperTest {
	...
	public void testToSysSequence() {
		SysSequence sequence = SysSequence.builder().sequenceName("TestSeq").initValue(1).maxValue(10000).currentValue(1)
		...
		Assert.assertEquals(sequence.getSequenceName(), sequenceDto.getSequenceName());

	}

	@Test
	public void testToSysSequenceDTO() {
		SysSequenceDTO sequenceDto = SysSequenceDTO.builder().sequenceName("TestSeq").initValue(1).maxValue(10000).currentValue(1)
				.stepValue(1).currDate("2019-01-01").preFixString("CRU").postFixString("End").comments("Test").build();
